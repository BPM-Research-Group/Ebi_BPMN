...
impl Recognisable for TagManualTask {
        ...
        if n.is_bpmn() {
            match state.open_tags.iter().last() {
                Some(OpenedTag::Process { .. }) | Some(OpenedTag::SubProcess { .. }) => {
                    if e.local_name().as_ref() == b"manualTask" {
                        return Some(Tag::ManualTask);
                    }
                }
                _ => {}
            }
...
impl Openable for TagManualTask {
...
impl Closeable for TagManualTask {
        ...
        match state.open_tags.iter_mut().last() {
            ...
            | Some(OpenedTag::SubProcess { elements, .. }) => {
                ...
                } = opened_tag
                {
                    ...
                    elements.push(BPMNElement::ManualTask(BPMNManualTask {
                        global_index,
                        id,
                        local_index,
                        activity,
