...
pub(crate) struct TagReceiveTask {}

impl Recognisable for TagReceiveTask {
        ...
        if n.is_bpmn() {
            match state.open_tags.iter().last() {
                Some(OpenedTag::Process { .. }) | Some(OpenedTag::SubProcess { .. }) => {
                    if e.local_name().as_ref() == b"receiveTask" {
                        return Some(Tag::ReceiveTask);
                    }
                }
                _ => {}
            }
...
impl Openable for TagReceiveTask {
...
impl Closeable for TagReceiveTask {
    fn close_tag(
        ...
        match state.open_tags.iter_mut().last() {
            ...
            | Some(OpenedTag::SubProcess { elements, .. }) => {
                ...
                } = opened_tag
                {
                    ...
                    elements.push(BPMNElement::ReceiveTask(BPMNReceiveTask {
                        global_index,
                        id,
                        local_index,
                        activity,
                    ...
                    return Err(anyhow!("Expected a receive task."));
                }
            }
